...
export function DayMatches({ matches, pickForMatch, onPick, readOnly = true, bypassLock = false }: DayMatchesProps) {
  return (
      ...
      {groupMatchesByDay(matches).map((day) => (
        <section key={day.date} className="space-y-5">
          ...
          {day.rounds.map((group) => (
            <RoundSection key={group.round.name} group={group} pickForMatch={pickForMatch} onPick={(id, side) => onPick?.(id, side)} readOnly={readOnly} bypassLock={bypassLock} />
          ))}
        </section>
      ))}
    </div>
